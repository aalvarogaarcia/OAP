...
def get_experiment_config():
    ...
    base_questions = [
        inquirer.Text(
            'instance_name', 
            message="Nombre de la instancia (ej. london-0000020)", 
            default="london-0000020"
        ),
        inquirer.List(
            'model_type',
            message="¿Qué modelo deseas ejecutar?",
            choices=['Compacto', 'Benders'],
            default='Benders'
        ),
        inquirer.Confirm(
            'maximize',
            message="¿Deseas maximizar la función objetivo? (Si no, se minimizará)",
            default=True
        ),
        inquirer.Confirm(
            'relaxed',
            message="¿Deseas resolver la relajación lineal del modelo?",
            default=False
        ),
        inquirer.Confirm(
            'polihedral',
    ...
    if config['model_type'] == 'Benders':
        benders_questions = [
            ...
            inquirer.Confirm(
                'modify_log_path',
                message="¿Deseas modificar la ruta predeterminada del log de cortes?",
                default=False
            )
        ]
...
def main():
    ...
    # 2. CARGA DE DATOS
    # ---------------------------------------------------------
    filepath = f"instance/{instance_name}.instance"
    try:
    ...
    if config['model_type'] == 'Compacto':
        ...
        modelo.solve(relaxed=config['relaxed'], save_cuts=config['save_cuts'], verbose=True, polihedral=config['polihedral'])
