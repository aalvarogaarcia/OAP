...
class BendersAnalysisMixin:
    ...
    def generate_combinatorial_report(
        self, output_pdf: str | None = None, max_vars: int = 15, top_k_cases: int = 100
    ) -> None:
        """
            ...
            for i, log in enumerate(logs):
                ...
                for comb in itertools.product([0, 1], repeat=len(all_vars)):
                    ...
                    for e1, e2 in itertools.combinations(active_arcs, 2):
                        if segments_intersect(
                            self.points[e1[0]], self.points[e1[1]], self.points[e2[0]], self.points[e2[1]]
                        ):
                            has_intersect = True
                            break
                    if has_intersect:
                ...
                for j, (case_dict, active_edges, s_l, s_r, act_izq, act_der) in enumerate(valid_cases[:top_k_cases], 1):
                    ...
                    nx.draw_networkx_nodes(
                        G, posiciones, node_size=600, node_color="lightblue", edgecolors="black", ax=ax
                    )
                    nx.draw_networkx_labels(G, posiciones, font_size=11, font_weight="bold", ax=ax)

                    nx.draw_networkx_edges(
                        G, posiciones, edgelist=active_edges, edge_color="green", width=3, arrowsize=20, ax=ax
                    )
                    nx.draw_networkx_edges(
                        G,
                        posiciones,
                    ...
                    str_const = (
                        f" + {constant_right:.2f}"
                        if constant_right > 0
                        else (f" - {abs(constant_right):.2f}" if constant_right < 0 else "")
                    )

                    titulo = (
                        f"Corte {i + 1} (Iter {log['iteration']}) | Caso {j}/{len(valid_cases)}\n"
