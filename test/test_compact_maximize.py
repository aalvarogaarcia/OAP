...
def test_oap_compact_model_maximize(ref_data, caplog):  # type: ignore[no-untyped-def]
    ...
    # Capturamos GurobiError de licencia (size-limited) y hacemos skip: esto
    # ocurre en máquinas sin licencia académica completa cuando la instancia
    # (n≥20) supera el límite de variables/restricciones de la licencia gratuita.
    try:
        modelo.solve(time_limit=300, verbose=False)
    except gp.GurobiError as exc:
        if "size-limited" in str(exc).lower() or "too large" in str(exc).lower():
            pytest.skip(f"Instance '{instance_name}' too large for Gurobi size-limited license: {exc}")
        raise
    ...
    # Para maximización, LP es cota superior: solo falla si el calculado es MENOR que la referencia.
    if lp_val != "-":
        assert lp_val >= ref_data["MAX_LPvalue"] * (1 - 1e-4), (
            f"ERROR: La relajación lineal (LP) para {instance_name} es peor que la referencia. LP Calculado: {lp_val:.4f} vs TSV: {ref_data['MAX_LPvalue']}"
        )

    # 4. (Opcional) Verificar explícitamente el Gap
    # Para maximización, un gap menor o igual que el de referencia (±0.5 pp) es aceptable.
    if gap != "-":
        assert gap <= ref_data["MAX_LPgap"] + 0.5, (
