...
def run_batch(
    ...
    if not filter_texts:  # Si la lista está vacía (no se usó el flag)
        ...
        instancias_filtradas = [p for p in todas_instancias if any(texto in p.name for texto in filter_texts)]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--time", type=int, default=300, help="Límite de tiempo en segundos para cada ejecución (default: 300)."
    )
    parser.add_argument(
        "-m",
        "--method",
    ...
    parser.add_argument(
        "--maximize", action="store_true", help="Si se incluye este flag, el modelo maximizará. Por defecto minimiza."
    )
    parser.add_argument(
        "--lp",
        action="store_true",
