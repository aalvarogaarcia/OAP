    ...
    for comb in itertools.product([0, 1], repeat=len(all_vars)):
        ...
        for edge1, edge2 in itertools.combinations(active_arcs, 2):
            if segments_intersect(
                posiciones[edge1[0]], posiciones[edge1[1]], posiciones[edge2[0]], posiciones[edge2[1]]
            ):
...
def mostrar_casos_secuencialmente(casos, posiciones):
    ...
    for i, (case_dict, s_l, s_r) in enumerate(casos, 1):
        ...
        nx.draw_networkx_edges(
            G, posiciones, edgelist=inactive_edges, edge_color="grey", width=1, style="dashed", alpha=0.3
        )

        # 3. Armamos el título de forma limpia
        plt.title(
            f"Caso {i}/{len(casos)} | Ordenado por MAX Lado Der\n[{str_izq}] ({s_l})  >=  [{str_der}] ({s_r})",
            fontsize=12,
        )
        plt.axis("off")
